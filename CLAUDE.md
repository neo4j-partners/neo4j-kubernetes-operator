...
74. **PVC seedBackupRef requires `URLConnectionSeedProvider` in cluster `dbms.databases.seed_from_uri_providers`**: Neo4j's default is `CloudSeedProvider` only — http:// URIs from the in-cluster PVC seed proxy fail with `Neo.ClientError.Statement.ArgumentError (No seed providers found to satisfy the provided uri 'http://…')` until the cluster's spec.config explicitly lists `URLConnectionSeedProvider`. The PVC branch of the sharded controller calls `EnsureClusterHasURLSeedProvider` (in `internal/controller/cluster_seed_providers.go`) right after the seed-creds projection check. Default behavior is actionable-error with a copy-pasteable snippet pointing at the cluster's spec.config; annotation `neo4j.com/auto-enable-url-seed-provider=true` on the cluster CR authorises the operator to auto-patch (triggers a rolling restart so Neo4j picks up the ConfigMap change). Mirrors the auto-inherit-seed-creds pattern from rule 61 — same rationale: a sharded-DB controller shouldn't restart cluster pods unsolicited. The PVC F5 integration test pre-configures the provider list explicitly to exercise the no-op branch and skip the restart cycle; unit tests cover the actionable-error and auto-enable branches. Cloud-backed seedBackupRef paths don't need this — the shipped default already includes CloudSeedProvider.
75. **`joinProviderList` preserves the shipped default when extending**: when `dbms.databases.seed_from_uri_providers` is unset (nil config) and the operator auto-enables `URLConnectionSeedProvider`, the resulting value is `"CloudSeedProvider,URLConnectionSeedProvider"` — NOT just `"URLConnectionSeedProvider"`. Silently dropping CloudSeedProvider would break any existing cloud-backed seed URIs the user might also be using. The helper dedupes (case-sensitive, whitespace-tolerant) so repeated invocations don't keep appending.
