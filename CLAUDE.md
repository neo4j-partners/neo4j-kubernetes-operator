...
76. **Restore `--from-path` resolves to a FILE via shell substitution, not a directory**: `buildLocalRestoreFilePath` emits `$(ls /backup/<run>/<dbname>-*.backup | head -1)` so neo4j-admin 5.26+ gets a file path (the only form it accepts — see [recreate-database docs](https://neo4j.com/docs/operations-manual/5/database-administration/standard-databases/recreate-database/) for the `--from-path` contract). The operator doesn't know the timestamp at reconcile time; the shell resolves it at Pod startup. This ALSO sidesteps the multi-`.backup` files in cluster-target backup directories. Cloud URIs (`s3://`, `gs://`, `azb://`) skip this — neo4j-admin's native cloud readers handle per-file selection. Never revert to passing the directory; never substitute the timestamp in Go.
