---
name: issue-hygiene
description: Keep GitHub issues honest — dedupe before filing, link every PR to its issue, and close on the right trigger (maintainer-filed work closes on merge; externally-reported issues stay open until verified on a released version).
---

# GitHub issue hygiene

Repo: `priyolahiri/neo4j-kubernetes-operator`. This skill encodes the issue/PR
...
| An **external reporter / community contributor** | **Keep OPEN**; add the `ready for review` label and move it to *Ready for review* on the project board | The reporter re-tests against the **next released version** the maintainer cuts, then signs off. Merge ≠ verified. |

The external-reporter path is the release-verification flow: a community-reported
issue is only truly done once it's confirmed against a *released* version, not the
moment code lands on `main`. Don't auto-close it just because the PR merged.
...
1. Search open *and* closed issues for the symptom before creating anything:
   ```bash
   gh issue list  --repo priyolahiri/neo4j-kubernetes-operator \
                  --search "backup pvc owner-ref" --state all --limit 30
   gh search issues --repo priyolahiri/neo4j-kubernetes-operator \
...
2. File with a focused title and the matching template
   ...
   ```bash
   gh issue create --repo priyolahiri/neo4j-kubernetes-operator \
...
3. Use a Conventional Commit title that names the issue, matching the established
   ...
   ```bash
   gh pr create --repo priyolahiri/neo4j-kubernetes-operator \
   ...
   For an *externally-reported* issue, deliberately do NOT use a closing keyword
...
4. Identify the issue author, then branch on the policy table above:
   ```bash
   gh issue view <NN> --repo priyolahiri/neo4j-kubernetes-operator \
     --json author,title,labels -q '.author.login'
   ```

5. **Maintainer-filed → close, always with a reason:**
   ```bash
   gh issue close <NN> --repo priyolahiri/neo4j-kubernetes-operator \
     --reason completed \
...
6. **Externally-reported → keep open, mark ready for verification:**
   ```bash
   gh issue edit <NN> --repo priyolahiri/neo4j-kubernetes-operator \
     --add-label "ready for review"
   gh issue comment <NN> --repo priyolahiri/neo4j-kubernetes-operator \
     --body "Fixed by #<PR>, merged to main. Please re-test and confirm once \
   it ships in the next released version."
   ```
   It closes only after the reporter verifies on the released version the
   maintainer later cuts (see the `cut-release` skill's post-release wrap-up,
   which adds the verification comment).

## Guardrails

- **Dedupe first, every time.** A closed-as-duplicate issue is cheap; a parallel
  thread that splits the discussion is expensive.
- **No closing keyword on externally-reported issues' PRs.** GitHub's auto-close
  would pull the issue closed on merge and silently break the verification policy.
...
"done" issues close on the merge that fixes them, while externally-reported
issues stay open under a `ready for review` label until the reporter re-tests
against a freshly *released* version. Merge is the done-signal for the author who
