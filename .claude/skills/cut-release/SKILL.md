...
- A tag, once pushed, publishes to GHCR (signed), the Helm OCI + classic repo,
  a GitHub Release, and versioned docs. If something is wrong
...
5. **Tag and push** (only after step 4):
   ...
   Helm OCI push) → `create-release` (GitHub Release) → `publish-docs`
   (`pages-docs.yml`) + `publish-helm` (`pages-helm.yml`, classic repo).
...
6. **Monitor the release run end-to-end.**
   ...
   settle green through `publish-docs` / `publish-helm`.
...
7. **Post-release wrap-up:**
   ...
   - Announce the release (GitHub Release notes / Discussions).
   - Add verification comments on externally-reported issues that this release
     addresses (so the reporter can confirm against the released version).
