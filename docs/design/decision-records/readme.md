...
| [BDR-014](business/backup-restore/014-backup-restore.md) | Backup / restore — **Option A proposed**: three CRDs `Neo4jBackup` + `Neo4jBackupSchedule` + `Neo4jRestore`; object-store destinations; databases by name (`["*"]` incl. `system`); Enterprise only; V2 | proposed |
...
| [ADR-015](architecture/015-backup-and-restore.md) | Backup / restore execution — one Kubernetes `Job` per run (neo4j image), credentials in the Job, restore gated on formation | proposed |
