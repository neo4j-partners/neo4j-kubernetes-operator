...
- **Chain aggregation** (`source.aggregate: true`) collapses a full+differential chain into a single recovered full artifact for a faster restore (lower RTO). The restore controller runs a pre-seed `neo4j-admin backup aggregate` Job (owned by the `Neo4jRestore`, `--keep-old-backup=true` so the chain is never destroyed) over the same backups claim, records the recovered artifact's filename via `/dev/termination-log`, then seeds `file:/backups/<recovered>`. It is PVC-backed-backupRef only (the Job needs filesystem access to the claim). Chain-seed (the default) needs no aggregation but pays the recovery cost at seed time. States surface as `reason=RestoreAggregating` (running) / `RestoreAggregateFailed` (terminal).
