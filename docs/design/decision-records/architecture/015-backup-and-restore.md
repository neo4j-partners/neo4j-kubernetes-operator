...
- **Chain aggregation** (`neo4j-admin backup aggregate`) collapses a full+differential chain into a single recovered full artifact for a faster restore (lower RTO); it is a separate step tracked as a follow-up. Chain-seed (above) needs no aggregation but pays the recovery cost at seed time.
