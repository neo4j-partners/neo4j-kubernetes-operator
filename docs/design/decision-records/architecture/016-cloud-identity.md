...
**Chain identity — generated, never typed.** A chain id flows into four places (the key prefix, `status.chain`, the retention-grouping label, and derived object names like `<chain>-agg`), so it must be simultaneously a valid object-store key segment, a valid label value, and a valid DNS-1123 object name. That invariant only holds if the id is *generated*, so we never expose a free-form chain field to users:
