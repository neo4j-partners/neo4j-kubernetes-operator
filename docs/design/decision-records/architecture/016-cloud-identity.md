...
- **Workload identity is instance-level, not per-backup.** Cloud IAM trust (IRSA trust policy, GKE IAM binding, Azure federated credential) binds to a **fixed ServiceAccount name+namespace**, so the identity cannot be chosen per `Neo4jBackup`. There is therefore no `destination.workloadIdentity`; WI lives on `spec.security.cloudIdentity`, and the same instance identity serves both the backup write and the restore-seed read of the same store.
