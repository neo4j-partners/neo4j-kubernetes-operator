...
A schedule may run several chains into one bucket over time (each `Full` anchors a new chain, differentials attach to it). A differential cannot attach to the *wrong* chain — it is contiguous by transaction id, so neo4j-admin always extends the **latest** full — and restore/aggregate over a folder resolve the latest chain. The reason to isolate is **per-chain addressability for automation**: the operator must aggregate a *specific* closed chain and delete a *specific* expired chain's objects **by prefix** (the retention prune `purge`s `<url>/<chainId>/`). Co-mingled in one flat prefix, `--from-path=<folder>` could only ever aggregate/restore the *latest* chain, and there would be no prefix to purge a single expired chain by. So a schedule isolates each chain in its own sub-directory (`render/backup/job.go` `chainSubDir`) on both PVC and object stores — the backup Job writes to `<url>/<chainId>/` (a key prefix; no `mkdir`, the store materializes it on write). One helper (`render/backup.ObjectStoreFolder`) computes that folder for both the **write** path (`--to-path`) and the **record** path (`status.artifacts[].uri`), so restore-by-`backupRef` and object-store aggregate resolve the exact prefix the backup wrote to.
