...
- **Target**: `Neo4j.spec.topology.mode` (`Cluster` when ≥3), `spec.topology.primaries.members`, `spec.topology.minimumMembers` — optional and immutable; when unset the operator derives `dbms.cluster.minimum_initial_system_primaries_count` as 1 for a single primary and 3 otherwise, so scaling never rewrites it (BDR-002 amendment)
