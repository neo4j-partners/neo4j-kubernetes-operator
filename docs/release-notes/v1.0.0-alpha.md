...
- The operator image is now signed with **Cosign keyless OIDC** signing on every release. (The MCP server image is the official `mcp/neo4j` published by Neo4j — the operator no longer builds or pushes a custom MCP image.)
