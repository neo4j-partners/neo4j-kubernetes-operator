...
| cert-manager `dnsNames` = cluster DNS ∪ optional `dnsNames` ∪ ingress hosts when `includeIngressHosts` | Ingress pass-through / external hostname coverage |
| Ingress hosts apply to **bolt** and **https** Certificates only — not **cluster** | Cluster SANs are inter-member discovery DNS |

---

## cert-manager — `dnsNames` derivation

When `certManager.enabled: true`, the operator creates one cert-manager `Certificate` per configured policy. **`spec.dnsNames`** is assembled as follows:

| Source | When included | Policies |
|--------|---------------|----------|
| Operator-derived in-cluster DNS | always | bolt, https, cluster |
| `trust.certManager.dnsNames[]` | when set | bolt, https (not cluster) |
| `trust.certificates.{policy}.dnsNames[]` | when set | that policy only |
| `spec.connectivity.ingress.rules[].host` | when `trust.certManager.includeIngressHosts: true` | bolt, https only |

**`includeIngressHosts`** defaults **`false`**. Set `true` when Neo4j terminates TLS and clients reach the database via an Ingress hostname (TLS pass-through or direct HTTPS/Bolt routing). Requires at least one `spec.connectivity.ingress.rules[].host` (TLS-007).

Ingress is defined under **`spec.connectivity.ingress`** ([BDR-007](006-service-exposure-connectivity.md) — operator-managed Ingress when enabled). Hosts are **read-only inputs** for Certificate SANs; the operator does not duplicate Ingress TLS Secrets unless a future BDR adds Ingress-terminated TLS.
...
trust:
  certManager:
    ...
    issuerRef:
      ...
      kind: ClusterIssuer
    includeIngressHosts: true    # default: false — merge connectivity.ingress.rules[].host into bolt/https SANs
    dnsNames:                    # optional extra SANs (e.g. external LB hostname)
...
| — | `certManager.includeIngressHosts`, `certManager.dnsNames` |
| — | `connectivity.ingress.rules[].host` → SANs when `includeIngressHosts` |
...
spec:
  ...
  dnsNames:
    ...
    - prod-0.prod.neo4j.svc.cluster.local    # operator-derived cluster DNS
    - bolt.prod.example.com                  # certManager.dnsNames
    - neo4j.example.com                      # connectivity.ingress.rules[].host (includeIngressHosts)
    - bolt.neo4j.example.com
  privateKey:
    algorithm: RSA
    size: 2048
...
**Bolt** Certificate gets the same ingress hosts when `includeIngressHosts: true`. **Cluster** Certificate uses member/discovery DNS only — ingress hosts excluded.
...
- [`validation.md`](../../09-crd-spec/neo4j/validation.md) — TLS-001…007
- [BDR-007](006-service-exposure-connectivity.md), [BDR-008](008-neo4j-config-surface.md), [BDR-011](011-https-connector-tls-coupling.md)
