...
The operator will **not** replicate “one release per member”. It uses **one `Neo4j` CR** fanning out to **pool StatefulSets** in Cluster mode ([BDR-009](009-scale-pool-ordinal-semantics.md) Option B). Topology fields express intent per pool in that unified model.
...
>
> **Amendment 2026-06-22 (pool shape)** — `secondaries[]` list with free `name` replaced by **fixed object keys** **`analytics`** and **`read`**. Removes duplicate-name validation; encodes read vs analytics intent in the schema.
>
> **Amendment 2026-06-22 ([BDR-009](009-scale-pool-ordinal-semantics.md) Option B — accepted)** — Cluster mode uses **one StatefulSet per pool** (`<name>-primary`, `<name>-analytics`, `<name>-read`), not one contiguous STS. Each pool has its own ordinal namespace `0..members-1`. Standalone: single STS, one replica. Supersedes the single-STS ordinal layout below.
...
**StatefulSets** ([BDR-009](009-scale-pool-ordinal-semantics.md) Option B — accepted):

| Mode | Workloads |
|------|-----------|
| `Standalone` | One STS `<name>`, `replicas: 1` |
| `Cluster` | Up to three STSs — `<name>-primary`, `<name>-analytics` (if `secondaries.analytics.members > 0`), `<name>-read` (if `secondaries.read.members > 0`) |

Each STS ordinals are **local to the pool** (`0 .. members-1`). Pod name: `<sts-name>-<ordinal>` (e.g. `prod-read-2`).

**Superseded (single STS):** one contiguous ordinal layout primaries → analytics → read — see BDR-009 history.
