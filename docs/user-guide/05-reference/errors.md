...
| Error | VersionDowngradeRefused | error | condition+event | `spec.version` moves backwards. Neo4j supports no downgrade in any form, and reverting the field does not undo a store the newer binary has already opened — the documented recovery is restoring a backup taken before the upgrade (ADR-017) |
| Error | VersionUpgradeRefused | error | condition+event | A `spec.version` change the operator will not roll out as written: an upgrade path that skips an LTS checkpoint, a digest-pinned image where the new version would never be pulled, offline maintenance, or plugin JARs on an `Existing` volume that nothing refreshes. The message names which (ADR-017) |
...
| BackupReady | BackupSourceUnsupported | error | condition+event | The aggregate source cannot be used (missing recorded artifact, or databases spread across mixed stores/claims/urls) |
