...
operator does not invent a sub-folder. So a full and its later incrementals **must point at the same
`url`** to form one chain: the incremental extends the full sitting in that folder, and an
`Aggregate` (below) collapses them there. To run two independent chains by hand, point each at a
**distinct `url`** — keeping them apart is yours to get right (put two unrelated chains in one folder
and an increment can attach to the wrong full). Restore and aggregate resolve the exact folder from
