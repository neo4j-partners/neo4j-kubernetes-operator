...
// +kubebuilder:validation:XValidation:rule="!has(self.topology.minimumMembers) || self.topology.minimumMembers > 1 || (has(self.topology.primaries) && has(self.topology.primaries.members) && self.topology.primaries.members == 1)",message="minimumMembers 1 requires primaries.members 1 (a multi-primary system database cannot bootstrap on a single server)"
...
// the per-pool lists. Kept in step with plugins.EnterpriseOnlyIDs by a test (BDR-004).
// +kubebuilder:validation:XValidation:rule="self.edition != 'community' || !has(self.plugins) || self.plugins.all(p, p != 'bloom' && p != 'fleet-management' && p != 'genai' && p != 'gds')",message="bloom, fleet-management, genai and gds ship only in the enterprise image; set edition to enterprise or drop the plugin"
...
// +kubebuilder:validation:XValidation:rule="!has(self.trust) || !has(self.trust.certManager) || !has(self.trust.certManager.includeIngressHosts) || self.trust.certManager.includeIngressHosts != true || (has(self.connectivity) && has(self.connectivity.ingress) && has(self.connectivity.ingress.rules) && self.connectivity.ingress.rules.exists(r, has(r.host) && r.host != ''))",message="includeIngressHosts requires at least one connectivity.ingress.rules[].host"
// +kubebuilder:validation:XValidation:rule="self.topology.mode != 'Cluster' || !has(self.trust) || !has(self.trust.enabled) || self.trust.enabled != true || (has(self.trust.certificates) && has(self.trust.certificates.cluster))",message="cluster TLS is required for clustered deployments when trust is enabled"
// +kubebuilder:validation:XValidation:rule="self.topology.mode != 'Standalone' || !has(self.trust) || !has(self.trust.enabled) || self.trust.enabled != true || !has(self.trust.certificates) || !has(self.trust.certificates.cluster)",message="trust.certificates.cluster is only valid when topology.mode is Cluster"
// +kubebuilder:validation:XValidation:rule="self.topology.mode != 'Standalone' || !has(self.trust) || !has(self.trust.enabled) || self.trust.enabled != true || (has(self.trust.certificates) && (has(self.trust.certificates.bolt) || has(self.trust.certificates.https)))",message="Standalone trust.enabled requires trust.certificates.bolt and/or trust.certificates.https"
// +kubebuilder:validation:XValidation:rule="self.topology.mode != 'Cluster' || (has(self.trust) && ((has(self.trust.enabled) && self.trust.enabled == true && has(self.trust.certificates) && has(self.trust.certificates.bolt)) || (has(self.trust.insecureAdminConnection) && self.trust.insecureAdminConnection == true)))",message="Cluster mode needs an admin Bolt path for the operator: set trust.certificates.bolt with trust.enabled: true (verified TLS) or trust.insecureAdminConnection: true (NEO-004)"
// +kubebuilder:validation:XValidation:rule="!has(self.connectivity) || !has(self.connectivity.multiCluster) || !has(self.connectivity.multiCluster.enabled) || self.connectivity.multiCluster.enabled != true",message="multi-cluster not in V1"
...
// +kubebuilder:validation:XValidation:rule="!has(self.connectivity) || !has(self.connectivity.service) || !has(self.connectivity.service.expose) || self.connectivity.service.expose.exists(e, e == 'bolt')",message="connectivity.service.expose must include bolt (the operator manages Neo4j over Bolt via the client Service); omit the field entirely to keep the default [bolt, http]"
