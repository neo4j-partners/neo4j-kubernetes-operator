...
// +kubebuilder:validation:XValidation:rule="!has(self.connectivity) || !has(self.connectivity.service) || !has(self.connectivity.service.expose) || size(self.connectivity.service.expose) == 0 || self.connectivity.service.expose.exists(e, e == 'bolt')",message="connectivity.service.expose must include bolt (the operator manages Neo4j over Bolt via the client Service)"
