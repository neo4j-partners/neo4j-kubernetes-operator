...
// +kubebuilder:validation:XValidation:rule="self.topology.mode != 'Standalone' || !has(self.trust) || self.trust.enabled != true || !has(self.trust.certificates) || !has(self.trust.certificates.cluster)",message="trust.certificates.cluster is only valid when topology.mode is Cluster"
// +kubebuilder:validation:XValidation:rule="self.topology.mode != 'Standalone' || !has(self.trust) || self.trust.enabled != true || (has(self.trust.certificates) && (has(self.trust.certificates.bolt) || has(self.trust.certificates.https)))",message="Standalone trust.enabled requires trust.certificates.bolt and/or trust.certificates.https"
