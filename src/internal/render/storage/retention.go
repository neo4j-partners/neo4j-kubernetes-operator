...
func RetentionPolicy(neo4j *neo4jv1.Neo4j) *appsv1.StatefulSetPersistentVolumeClaimRetentionPolicy {
	whenDeleted := appsv1.RetainPersistentVolumeClaimRetentionPolicyType
	whenScaled := appsv1.RetainPersistentVolumeClaimRetentionPolicyType
	if EffectiveWhenDeleted(neo4j) == neo4jv1.VolumeClaimRetentionDelete {
	...
	if neo4j != nil && neo4j.Spec.Storage != nil && neo4j.Spec.Storage.VolumeClaimRetention != nil {
		...
		if r.WhenScaled == neo4jv1.VolumeClaimRetentionDelete {
...
func SpecWhenDeleted(neo4j *neo4jv1.Neo4j) neo4jv1.VolumeClaimRetentionPolicyType {
	if neo4j != nil &&
		neo4j.Spec.Storage != nil &&
		neo4j.Spec.Storage.VolumeClaimRetention != nil &&
		neo4j.Spec.Storage.VolumeClaimRetention.WhenDeleted == neo4jv1.VolumeClaimRetentionDelete {
		return neo4jv1.VolumeClaimRetentionDelete
	}
	return neo4jv1.VolumeClaimRetentionRetain
}
...
func EffectiveWhenDeleted(neo4j *neo4jv1.Neo4j) neo4jv1.VolumeClaimRetentionPolicyType {
...
func PinWhenDeleted(neo4j *neo4jv1.Neo4j) bool {
	if neo4j == nil || neo4j.Status.VolumeClaimRetentionWhenDeleted != nil {
		return false
	}
...
func DeleteDataOnUninstall(neo4j *neo4jv1.Neo4j) bool {
	if neo4j == nil || neo4j.Status.VolumeClaimRetentionWhenDeleted == nil {
		return false
	}
	return *neo4j.Status.VolumeClaimRetentionWhenDeleted == neo4jv1.VolumeClaimRetentionDelete
}
...
func DeleteDataOnScale(neo4j *neo4jv1.Neo4j) bool {
	return neo4j != nil &&
		neo4j.Spec.Storage != nil &&
		neo4j.Spec.Storage.VolumeClaimRetention != nil &&
		neo4j.Spec.Storage.VolumeClaimRetention.WhenScaled == neo4jv1.VolumeClaimRetentionDelete
}

// ProtectedClaimNames are Existing.claimName PVCs the operator must never delete.
func ProtectedClaimNames(neo4j *neo4jv1.Neo4j) map[string]struct{} {
	out := map[string]struct{}{}
	if neo4j == nil || neo4j.Spec.Storage == nil || neo4j.Spec.Storage.Volumes == nil {
		return out
	}
	vols := neo4j.Spec.Storage.Volumes
	addExistingClaim := func(ex *neo4jv1.ExistingVolumeSpec) {
		if ex != nil && ex.ClaimName != "" {
			out[ex.ClaimName] = struct{}{}
		}
	}
	if vols.Data.Mode == neo4jv1.VolumeModeExisting {
		addExistingClaim(vols.Data.Existing)
	}
	for _, aux := range []*neo4jv1.AuxiliaryVolumeSpec{
		vols.Backups, vols.Logs, vols.Metrics, vols.Import, vols.Licenses, vols.Plugins,
	} {
		if aux != nil && aux.Mode == neo4jv1.VolumeModeExisting {
