...
func neo4jRejectedByValidation() *neo4jv1.Neo4j {
...
func TestFailedPipelineDoesNotAdvanceObservedGeneration(t *testing.T) {
	...
	stored.Status.Phase = neo4jv1.Neo4jPhaseRunning
	...
	if got.Status.Phase != neo4jv1.Neo4jPhaseFailed {
		t.Errorf("phase = %q, want %q", got.Status.Phase, neo4jv1.Neo4jPhaseFailed)
	}
