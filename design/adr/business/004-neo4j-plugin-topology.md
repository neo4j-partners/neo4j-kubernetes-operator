...
| **Date** | 2026-06-22 |
| **Depends on** | [BDR-002](002-neo4j-crd-topology.md) — `cores` + `replicas[]` |
...
| **GDS** | GDS Enterprise license (shared Secret — see invariants below) | **Forbidden on cores** — read replica or secondary / analytics server only |
| **Bloom** | Separate license per instance | Typically non-core |
| **APOC Extended** | Community / optional | Per-node choice |

In a cluster:

- **All core members** must share the same plugin set (homogeneous quorum members).
- **Non-core pools** may differ — e.g. read scaling with APOC only, another pool with **GDS + Bloom**.
- A **per-pod** plugin matrix (every replica × every combination) is unmaintainable.

[BDR-002](002-neo4j-crd-topology.md) already chose **`replicas[]`** (named pools + `serverRole`) for topology sizing. **This BDR decides how plugins attach to those pools** — and whether plugin **configuration** (license Secret, JAR version, `apoc.conf`) is colocated with assignment or factored out.
...
### 1. GDS must not run on cores

In **Cluster** mode, a cluster **cannot** have GDS installed on its **core** members. The operator MUST reject or refuse to reconcile any spec that assigns `gds` (or equivalent analytics-only plugins governed by the same Neo4j constraint) to `topology.cores`.

- GDS runs only on **non-core** pools: read replicas, secondaries, or dedicated analytics servers ([Neo4j GDS cluster deployment](https://neo4j.com/docs/graph-data-science/current/production-deployment/neo4j-cluster/)).
- Core pods may run plugins allowed on transactional members (e.g. APOC Core) but **not** GDS.
...
|------------|---------------|
| Simplest API — one list to learn | **Invalid for GDS on cores** — Neo4j forbids / unsupported |
...
| Minimal operator logic | **License compliance risk** — GDS license on core pods |
...
### Option B — Fixed counters: `readReplicas` + `readGDSReplicas`
...
| Advantages | Disadvantages |
|------------|---------------|
| Only two non-core buckets — bounded | **`readGDSReplicas` name implies GDS** — cannot express GDS + Bloom cleanly |
| Maps loosely to Helm analytics vs read scaling | **Cannot have two non-core pools** with different plugin sets |
| Smaller than arbitrary `replicas` | GDS + Bloom on one read replica **does not fit** |
| | Conflates topology sizing with plugin intent |
| | Superseded by `replicas[]` in BDR-002 amendment |
...
### Option C — `replicas[]` + separate `spec.plugins.<poolName>` map

Topology in `replicas`; plugin **assignment** keyed by pool name in a second map. Full `PluginSpec` (license, version, config) in the map.
...
| Supports GDS + Bloom on one pool | Must keep `replicas[].name` ↔ map key **in sync** |
| `cores` handled as `plugins.core` | Orphan / missing keys need validation (PLG-002 class) |
...
### Option D — `replicas[]` with inline `plugins[]` on each pool
...
| | Duplicates `PluginSpec` structure under `cores` and each pool |
| | Standalone needs a third shape (`spec.plugins[]` flat list) |

---

### Option E — `replicas[]` with plugin **refs** + central plugin definitions
...
**Plugin-centric model:** one entry per plugin id; each entry holds **configuration** (license, version, JAR) and an **`enabledOn`** list declaring **where** that plugin is installed. Targets are the literal `cores` and `replicas[].name` values.
...
| **DRY** — one `gds` entry with one `licenseSecretRef` (invariant §2) | `enabledOn` entries must match `replicas[].name` — sync / orphan validation (PLG-002) |
| `enabledOn: [cores]` maps cleanly to all core pods | Standalone needs a sentinel target (`standalone`) — extra convention |
| Natural validation: reject `gds.enabledOn` containing `cores` (invariant §1) | OpenAPI: fixed plugin keys vs extensible catalog — same as E |
| No separate `pluginDefinitions` section — fewer top-level fields than E | Unused plugin keys in map — warn or allow? |
| Easy license rotation — edit `plugins.gds` once | Homogeneous cores: all core pods get the same plugins listed under `enabledOn: [cores]` — cannot mix per-core plugin sets (Neo4j rule anyway) |

**Resolution rule (operator):** for each `plugins.<id>`, merge with catalog defaults; for each target in `enabledOn`, install on all pods in that pool (`cores` → core StatefulSet; pool name → matching `replicas[]`).
...
| Rule | Rationale |
|------|-----------|
| **`gds` MUST NOT be on `topology.cores`** — cluster cannot run GDS on core members | Neo4j GDS docs — cores serve transactional quorum; GDS is OLAP-only on non-core |
| `gds` / `bloom` not on core pods (same as above for other analytics-only plugins) | Plugin invariants §1 |
...
| `serverRole: Secondary` + `gds` → analytics server config | Neo4j 5.x GDS on secondary |

---

## Decision

**Not decided.** Options C, D, E, and F remain viable given [BDR-002](002-neo4j-crd-topology.md) `replicas[]`.
...
- [BDR-002](002-neo4j-crd-topology.md) — `replicas[]`
