...
### Option B — One StatefulSet per pool
...
**Not decided.** Pending PS review of the trade-off between keeping BDR-002's single-STS model and accepting day-2 scale limits.

**Proposer direction:** Avoid Option A — orchestrated ordinal migration with PVC reuse across roles is the highest-risk path and has no Helm precedent to validate against. Treat **role as immutable for a member's lifetime** and model reassignment as scale-in + scale-out in every option.

**Recommendation:** Adopt **Option B (one StatefulSet per pool)** as it structurally eliminates the ordinal-shift hazard, enables native per-pool `kubectl scale`, and aligns with Helm's "one workload per role" intent — accepting an amendment to BDR-002's "one STS" wording. If the single-STS object is considered non-negotiable for V1, fall back to **Option C (tail-only scaling, immutable primaries/analytics)** and defer HA promotion (`primaries 1→3`) and analytics resize to a documented maintenance procedure in V1.1+.
...
- `ENABLE SERVER` / `DEALLOCATE` automation replaces the Helm operations Job (`neo4j.operations.enableServer`) with no manual `cypher-shell`.
- Pool role immutability keeps BDR-004 plugin/placement invariants intact across scaling.

### Negative

- Options B and D **amend BDR-002**'s "one StatefulSet" statement and ripple into status server naming (`<name>-<pool>-<ordinal>`), Services, and PDBs.
- Option C limits day-2 elasticity to read replicas and pushes HA promotion to a maintenance flow.
- The `scale` subresource semantics (`spec.md` L539) must be pinned to the chosen option (aggregate vs per-pool vs read-only).
...
- [BDR-002](002-neo4j-crd-topology.md) — topology model (single STS, fixed ordinals)
