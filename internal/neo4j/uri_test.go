	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			cluster := &neo4jv1beta1.Neo4jEnterpriseCluster{
				...
				Spec:       neo4jv1beta1.Neo4jEnterpriseClusterSpec{AcceptLicenseAgreement: "eval", TLS: tc.tls},
			}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			standalone := &neo4jv1beta1.Neo4jEnterpriseStandalone{
				...
				Spec:       neo4jv1beta1.Neo4jEnterpriseStandaloneSpec{AcceptLicenseAgreement: "eval", TLS: tc.tls},
			}
