...
oracle_reasons_for() {  # <condition>
  case "$1" in
    ...
    event) printf '%s\n' RestoreMetadataConflict ScheduleBackupEmitted SchedulePruned SchedulePruneFailed SchedulePruneDelegated ScheduleCompacted ScheduleAggregateFailed DuplicateEntry DatabaseTopologyResized InsecureAdminConnection AdminBoltTLSRequired SecretMounted StorageResizeCompleted ;;
    *) return 1 ;;
  esac
}
...
oracle_severity() {  # <reason>
  case "$1" in
    ...
    SchedulePruneDelegated) echo info ;;
...
oracle_nominal() {  # <reason>
  case "$1" in
    AllMembersReady|InProgress|Completed|ObjectsCreated|NoError|PVCBound|TrustDisabled|SecretsPresent|Formed|NoDrain|BackupSucceeded|RestoreSucceeded|ScheduleActive|ScheduleBackupEmitted|SchedulePruned|SchedulePruneDelegated|ScheduleCompacted|SecretMounted|StorageResizeCompleted) return 0 ;;
    *) return 1 ;;
  esac
}
