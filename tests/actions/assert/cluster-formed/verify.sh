...
#     What it may hold instead is whatever the catalog lists for ClusterFormed
#     (tests/lib/oracle.sh), so the failure names the alternatives without keeping a copy here.
...
[[ "${formed_status}" == "True" ]] \
  || die "ClusterFormed=${formed_status:-<absent>} (reason=${formed_reason:-none}) — operator does not consider the cluster formed; catalogued reasons: $(oracle_reasons_for ClusterFormed | tr '\n' ' ')"
